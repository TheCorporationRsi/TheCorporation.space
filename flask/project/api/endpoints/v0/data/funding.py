...
def fund_dict():
    fundings = models.Funding.query.order_by(models.Funding.date).all()
...
def fund_year_dict(year):
    ...
    fundings = models.Funding.query.filter(and_(extract('year', models.Funding.date) == year, extract('hour', models.Funding.date) == 18)).order_by(models.Funding.date).all()
    list_funding = [r.as_dict() for r in fundings]
    return jsonify(list_funding)
...
def fund_year_hour_dict(year):
    fundings = models.Funding.query.filter(extract('year', models.Funding.date) == year).order_by(models.Funding.date).all()
    list_funding = [r.as_dict() for r in fundings]
    return jsonify(list_funding)
...
def fund_dict_graph(type):
    if type == "citizens":
        fundings = models.Funding.query.filter(and_(models.Funding.citizens != 0, models.Funding.citizens != None)).order_by(models.Funding.date).all()
    elif type == "both":
        fundings=models.Funding.query.filter(and_(models.Funding.fund != 0, models.Funding.fund != None, models.Funding.citizens != 0, models.Funding.citizens != None)).order_by(models.Funding.date).all()
    else:
        fundings=models.Funding.query.filter(and_(models.Funding.fund != 0, models.Funding.fund != None)).order_by(models.Funding.date).all()
    list_funding=[r.as_dict() for r in fundings]
    return jsonify(list_funding)
