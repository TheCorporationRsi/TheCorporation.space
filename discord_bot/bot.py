...
client.sio = socketio.AsyncClient(reconnection_delay=1, reconnection_delay_max=5, reconnection=True, reconnection_attempts=0)
...
async def on_reaction_add(reaction, user):
    ...
    if str(reaction.emoji) == '<:1INF:423754395944222721>':
        ...
        await client.sio.emit('influence_emote', {
        'sender_id': sender_id,
        'amount': 1,
    ...
    if len(before.roles) > len(after.roles):
            for role in before.roles:
                if role not in after.roles:
                    await client.sio.emit('role_removed', { 'user_id': before.id, 'role_id': role.id}, namespace='/discord_bot')
                    
    elif len(before.roles) < len(after.roles):
        for role in after.roles:
            if role not in before.roles:
                await client.sio.emit('role_added', { 'user_id': before.id, 'role_id': role.id}, namespace='/discord_bot')
...
@client.sio.on('dm_message', namespace='/discord_bot')
...
async def upload_role(data):
    ...
    for role in user.roles:
        await client.sio.emit('role_added', { 'user_id': user_id, 'role_id': role.id}, namespace='/discord_bot')
...
async def on_ready():
    ...
    await client.sio.connect(os.environ['FLASK_APP_URL'], namespaces=['/discord_bot'], auth={'api_key': os.environ['DISCORD_FLASK_API_KEY']}, retry=True)  
